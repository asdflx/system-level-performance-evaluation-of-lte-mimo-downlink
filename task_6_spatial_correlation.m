...
qos = ones(1, nUsers);
% user average rate
rate = cell(1, length(corSpatialConst));
%% System model
for iCorSpatialConst = 1: length(corSpatialConst)
    rate{iCorSpatialConst} = zeros(nDrops, nUsers);
    ...
    for iDrop = 1: nDrops
        ...
        % user distribution
        [dCenter, dInterf, corSpatial, corSpatialInterf] = user_distribution(dMin, dMax, nUsers, nInterfs, corSpatialConst(iCorSpatialConst));
        for iSample = 1: tDrop
        ...
        rate{iCorSpatialConst}(iDrop, :) = mean(instRate);
    end
    rate{iCorSpatialConst} = rate{iCorSpatialConst}(:);
end
