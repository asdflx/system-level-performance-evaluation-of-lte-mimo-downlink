...
qos = ones(1, nUsers);
% user average rate
rate = cell(1, length(tScale));
%% System model
for iScale = 1: length(tScale)
    rate{iScale} = zeros(nDrops, nUsers);
    ...
    for iDrop = 1: nDrops
        ...
        for iSample = 1: tDrop
            ...
            [avgRate, userIndex] = proportional_fair_scheduling(nUsers, cqi, avgRate, tScale(iScale), qos, iSample, userIndex);
            instRate(iSample, userIndex) = cqi(userIndex);
        end
        rate{iScale}(iDrop, :) = mean(instRate);
    end
    rate{iScale} = rate{iScale}(:);
end
